...
{
    // Prefer %TEMP% so the log is writable even when the exe is in Program Files.
    // Fall back to the exe directory for dev builds where %TEMP% isn't set.
    const char* tmpEnv = getenv("TEMP");
    if (!tmpEnv) tmpEnv = getenv("TMP");
    QString logPath;
    if (tmpEnv && *tmpEnv)
        logPath = QString::fromLocal8Bit(tmpEnv) + "/wavy_debug.log";
    else {
        QFileInfo exe(QString::fromLocal8Bit(argv0));
        logPath = exe.absolutePath() + "/wavy_debug.log";
    }
    g_logFile = fopen(logPath.toLocal8Bit().constData(), "w");
    if (g_logFile) {
        fprintf(g_logFile, "[logger] wavy_debug.log → %s\n", logPath.toLocal8Bit().constData());
        fflush(g_logFile);
        qInstallMessageHandler(wavyMessageHandler);
    }
}
