...
void BackendLauncher::start()
{
    ...
    // Use a dedicated temporary ZMQ socket so we don't interfere with
    // AIClient's async health-check thread (which also uses connectToBackend).
    {
        ...
        if (externalRunning) {
            // Defer signals so consumers (AIBackend, Console tab) have time
            // to wire their connections before ready/logLine arrive.
