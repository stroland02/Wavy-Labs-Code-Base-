...
void WavyShell::setAIPanel(QWidget* panel)
{
    ...
    if (splitter) {
        ...
        QTimer::singleShot(80, this, [this, splitter]() {
            const int n = splitter->count();
            QList<int> sz(n, 0);
            sz[0] = 48;                               // sidebar (icon bar)
            sz[1] = 326;                              // AI panel
            sz[n - 1] = qMax(600, splitter->width() - 392);  // workspace
            // Everything in between (hidden SideBarWidgets) stays at 0
            splitter->setSizes(sz);
            // Re-tile MDI subwindows after splitter settles
