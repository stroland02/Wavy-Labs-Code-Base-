...
void WavyShell::setAIPanel(QWidget* panel)
{
    ...
    // Find the horizontal QSplitter inside the LMMS content.
    // LMMS layout: m_sideBarSplitter contains [m_sideBar(48px), browsers..., m_workspace]
    QSplitter* splitter = m_lmmsContent ? m_lmmsContent->findChild<QSplitter*>() : nullptr;
    if (!splitter) {
        qWarning() << "[WavyShell] setAIPanel: QSplitter not found in LMMS content";
        return;
    }

    panel->setMinimumWidth(240);
    // Insert AI panel after the sidebar (index 0), before browsers/workspace
    splitter->insertWidget(1, panel);
    m_splitter = splitter;
    m_splitterSizesDone = false;

    // Apply sizes — splitter has no width until the window is shown and laid out.
    // Schedule at multiple intervals to handle slow machines and installed builds.
    auto applySizes = [this, splitter]() {
        ...
        QList<int> sz(n, 0);
        // sz[0] = 0: PluginBrowser stays collapsed (sidebar is outside the splitter)
        sz[1] = 326;                              // AI panel
        sz[n-1] = qMax(600, totalW - 326);        // workspace fills remainder
        splitter->setSizes(sz);
        QTimer::singleShot(0, this, [this]() { tileVisibleEditors(); });
    };

    // Five checkpoints: 50ms, 200ms, 500ms, 1000ms, 2000ms
    for (int ms : {50, 200, 500, 1000, 2000})
        QTimer::singleShot(ms, this, applySizes);
}
...
void WavyShell::setAIPanelVisible(bool visible)
{
    ...
    if (!visible)
        m_aiPanelWidth = qMax(260, m_aiPanel->width());

    m_aiPanel->setVisible(visible);

    const int n = m_splitter->count();
    const int totalW = m_splitter->width();
    QList<int> sz(n, 0);
    // sz[0] = 0: PluginBrowser stays collapsed (m_sideBar is outside the splitter in hbox)
    if (visible) {
        sz[1] = m_aiPanelWidth;
        sz[n-1] = qMax(600, totalW - m_aiPanelWidth);
    } else {
        sz[1] = 0;
        sz[n-1] = qMax(600, totalW);
    }
